...
def process_params(params):
    ...
    if result is not None:
        ...
        # Output current minimal diffs when no valid output is generated
        print(f"Current minimal diffs: diff1_v {global_min_diffs.get('min_diff1_v', 'N/A')}, diff2_v {global_min_diffs.get('min_diff2_v', 'N/A')}, diff1_h {global_min_diffs.get('min_diff1_h', 'N/A')}, diff2_h {global_min_diffs.get('min_diff2_h', 'N/A')}")
        return None
...
def check_strand_sequence_alignment(strands_dict, is_vertical, log_file=None):
    ...
    if min_length != float('inf'):
        if min_length - strand_width * 2 < 0:
            is_aligned = False
            # Always store lengths
            if is_vertical:
                store_valid_lengths_vertical(min_length, max_length, strand_width)
                ...
                store_valid_lengths_horizontal(min_length, max_length, strand_width)
            return is_aligned, dot_products, min_length, max_length
        if min_length - strand_width * 4 > 0:
            is_aligned = False
            # Always store lengths
            if is_vertical:
                store_valid_lengths_vertical(min_length, max_length, strand_width)
...
def check_valid_distances(vertical_strands, horizontal_strands, m, n):
    ...
    if m >= 1:  # Only check if there are vertical strands
        if '4' in vertical_strands[1] and '5' in vertical_strands[m]:
            ...
            if not ((expected_distance)/2  <= distance <= (expected_distance+2*n)*2):
                return False
    ...
    try:
        ...
        if not check_valid_distances(vertical_strands, horizontal_strands, m, n):
            ...
            if valid_lengths_v is not None:
                min_length_v = valid_lengths_v['min_length']
                max_length_v = valid_lengths_v['max_length']
                strand_width_v = valid_lengths_v['strand_width']
                diff1_v = min_length_v - strand_width_v * 2
                diff2_v = max_length_v - min_length_v

                # Update global_min_diffs
                updated = False
                ...
                if diff2_v < global_min_diffs.get('min_diff2_v', float('inf')):
                    ...
                    updated = True
                if updated:
                    print(f"Updated minimal diffs vertical: diff1_v {global_min_diffs['min_diff1_v']}, diff2_v {global_min_diffs['min_diff2_v']}")

            if valid_lengths_h is not None:
                min_length_h = valid_lengths_h['min_length']
                max_length_h = valid_lengths_h['max_length']
                strand_width_h = valid_lengths_h['strand_width']
                diff1_h = min_length_h - strand_width_h * 2
                diff2_h = max_length_h - min_length_h
                ...
                if diff2_h < global_min_diffs.get('min_diff2_h', float('inf')):
                    ...
                    updated = True
                if updated:
                    print(f"Updated minimal diffs horizontal: diff1_h {global_min_diffs['min_diff1_h']}, diff2_h {global_min_diffs['min_diff2_h']}")
...
def main():
    ...
    # Update total combinations calculation
    def calculate_total_combinations(m, n):
        return (
            len(vertical_angles) * 
    ...
    for m in m_values:
        for n in n_values:
            ...
            total_combinations = calculate_total_combinations(m, n)

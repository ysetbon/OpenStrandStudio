...
def calculate_point_to_line_distance_vectorized(point, line_start, line_end):
    ...
    # Convert dictionary inputs to numpy arrays if necessary
    if isinstance(point, dict):
        ...
        line_starts = np.array([line_start])
        line_ends = np.array([line_end])
...
def main():
    output_dir = r"C:\Users\YonatanSetbon\.vscode\OpenStrandStudio\src\samples\ver 1_073\all possible rh continulation twist offsets optimized numpy"
    ...
    # Create arrays for angles and offsets using numpy
    minimum_angle = 12
    maximum_angle = 65
    vertical_angles = np.arange(minimum_angle, maximum_angle + 1)
    horizontal_angles = np.arange(minimum_angle, maximum_angle + 1)
    possible_offsets = np.arange(min_offset, max_offset + 1, offset_step)
    
    # Create meshgrid for all combinations
    m_values = np.array([2])  # Since you're only using range(2,3)
    n_values = np.array([2])  # Since you're only using range(2,3)
    ...
    num_vertical_angles = len(vertical_angles)
    num_horizontal_angles = len(horizontal_angles)
    num_offsets = len(possible_offsets)
    ...
    with open(log_file_path, 'w') as log_file:
        # Process each angle combination
        for i_angle in vertical_angles:
            ...
            for j_angle in horizontal_angles:
                ...
                # Create offset combinations using numpy
                for v_offset in possible_offsets:
                    ...
                    for h_offset in possible_offsets:
                        ...
                        if json_content is not None:
                            filename = f"m{m_values[0]}_n{n_values[0]}_v{v_offset}_h{h_offset}_va{i_angle}_ha{j_angle}.json"
                        ...
                        # Update progress after each complete combination

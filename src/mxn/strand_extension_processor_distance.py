...
def extend_strands_to_distance(x4_strand, x5_strand, target_distance, is_horizontal, step_size=1):
    ...
    def calculate_distance():
        """Calculate signed perpendicular distance from x4 start to x5 line
        Returns positive distance if point is above the line, negative if below"""
        ...
        if abs(b[0] - a[0]) < 1e-6:  # If line is vertical
            # For vertical lines, positive means point is to the right
            return p[0] - a[0]
        
        # Remove the abs() to get signed distance

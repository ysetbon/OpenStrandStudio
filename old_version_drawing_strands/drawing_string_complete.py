...
class AttachedStrand:
    ...
    def draw(self, painter):
        ...
        start_point = QPointF(self.start.x() + perpendicular_dx - perpendicular_dx_stroke, self.start.y() + perpendicular_dy - perpendicular_dy_stroke)
        end_point = QPointF(self.start.x() - perpendicular_dx + perpendicular_dx_stroke, self.start.y() - perpendicular_dy + perpendicular_dy_stroke)
...
class StrandDrawingCanvas(QWidget):
    ...
    def handle_strand_attachment(self, pos):
        circle_radius = self.strand_width / 2
    ...
    def try_attach_to_strand(self, strand, pos, circle_radius):
        ...
        if distance_to_start <= circle_radius and strand.circles_available[0]:
            ...
            self.current_strand = AttachedStrand(strand, strand.start)
            strand.attached_strands.append(self.current_strand)
            ...
            self.current_strand = AttachedStrand(strand, strand.end)
            strand.attached_strands.append(self.current_strand)
            self.is_attaching = True
            return True
        return False
    ...
    def try_attach_to_attached_strands(self, attached_strands, pos, circle_radius):
        for attached_strand in attached_strands:
            ...
            if distance_to_end <= circle_radius and attached_strand.circle_available:
                ...
                self.current_strand = new_attached_strand
                self.is_attaching = True
                return True

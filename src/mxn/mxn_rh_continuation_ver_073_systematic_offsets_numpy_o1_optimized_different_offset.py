...
def main():
    ...
    for m in m_values:
        for n in n_values:
            output_dir = os.path.join(base_dir, f"m{m}xn{n}_rh_continuation_different_offset")
            ...
            with Pool(processes=cpu_count()) as pool:
                for processed_result in tqdm(
                    ...
                    if processed_result is not None:
                        ...
                        filename = f"m{m}_n{n}_{v_offset_str}_{h_offset_str}_va{i_angle}_ha{j_angle}_minlength_minus_strandwidth_{diff1:.2f}_maxlength_minus_minlength_{diff2:.2f}.json"

...
def generate_json(m, n):
    """
    RH "stretch" variant:
    Mirrors the changes made in `mxn_lh_strech.py`, but for RH orientation.

    Key differences vs baseline `mxn_rh.py`:
    - Uses "stretch geometry" (gap=±28, stride=112, length=stride)
    - Uses computed tail offsets (no hardcoded 82)
    - Uses a tighter vertical extent: half a stride beyond outermost horizontals
    """
    # Constants and parameters (STRETCH geometry)
    grid_unit = 42.0  # Grid unit size in pixels
    gap = -(grid_unit * (2.0 / 3.0))  # -28.0 (RH is negative)
    stride = 4.0 * abs(gap)  # 112.0
    length = stride
    # Keep the same global shift used by LH-strech so both variants line up in the same viewport
    ...
    for i in range(m):
        ...
        # Vertical extent for STRETCH (same rule as LH-strech)
        ...
        # RH vertical tails:
        # _2 is the TOP tail (attached at End), extends downward toward start_y
        ...
        # _3 is the BOTTOM tail (attached at Start), extends upward toward end_y
    ...
    for i in range(n):
        ...
        # Horizontal tails are the same convention as baseline:
        # _2 is right-attached, extends left
        ...
        # _3 is left-attached, extends right
    ...
    # Generate masked strands (same matching as baseline RH)
    ...
    shadow_overrides = {}
    if m == 1 and n == 1:
        # Same overrides as baseline RH
        shadow_overrides = {
            "1_3": {
                "1_1": {"visibility": True, "allow_full_shadow": True, "subtracted_layers": ["2_2", "2_3"]},
...
def main():
    ...
    output_dir = os.path.join(script_dir, "mxn_rh_stretch")
    os.makedirs(output_dir, exist_ok=True)

    with open(os.path.join(output_dir, "debug.txt"), "w") as f:
        f.write("Started RH STRETCH\n")

    for m in range(1, 4):
        for n in range(1, 4):
            try:
                json_content = generate_json(m, n)
                file_name = f"mxn_rh_stretch_{m}x{n}.json"
...
if __name__ == "__main__":
    print("Running main RH STRETCH...")
    main()
    print("Finished main RH STRETCH.")

...
def check_valid_distances(vertical_strands, horizontal_strands, m, n):
    """Check if distances between first x4 and last x5 are valid"""
    strand_width = 54  # Width of a strand - fixed value instead of undefined variable
    ...
    if m >= 1:  # Only check if there are vertical strands
        ...
        if '4' in vertical_strands[1] and '5' in vertical_strands[m]:
            ...
            expected_distance = n * strand_width
            # Allow some tolerance (±10% of expected distance)
            if not ((expected_distance+2*n)*1.98  <= distance <=(expected_distance+2*n)*2.02):
                return False
    ...
    if n >= 1:  # Only check if there are horizontal strands
        ...
        if '4' in horizontal_strands[m+1] and '5' in horizontal_strands[m+n]:
            ...
            expected_distance = m * strand_width 
            # Allow some tolerance (±10% of expected distance)
            if not ((expected_distance+2*m)*1.98 <= distance <= (expected_distance+2*m)*2.02):
                return False
...
def main():
    output_dir = r"C:\Users\YonatanSetbon\.vscode\OpenStrandStudio\src\samples\ver 1_073\all possible rh continulation twist offsets"
    ...
    for i in range(20, 55, 5):  # vertical angles
        for j in range(20, 55, 5):  # horizontal angles
            for v_x2_offset in range(max_offset,0, offset_step):
                for v_x3_offset in range(max_offset,0, offset_step):
                    for h_x2_offset in range(max_offset,0, offset_step):
                        for h_x3_offset in range(max_offset,0, offset_step):
                            x4_vertical_angle = i

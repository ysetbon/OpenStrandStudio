...
def generate_json(m, n):
    ...
    for i in range(m):
        ...
        # Top Horizontal Top Edge: h_top_cy + gap (gap is negative, so +gap is higher/smaller Y)
        # However, gap is offset from center line. 
        # If gap=-42, top edge is cy - 42.
        end_y = (h_top_cy + gap) - grid_unit - grid_unit/3
        
        # Vertical Start (Bottom): 4 grids below Bottom Horizontal's bottom edge
        # Bottom Horizontal Bottom Edge: h_bottom_cy - gap (gap is negative, so -gap is lower/larger Y)
        ...
        # Attached Strand (_2) - Top (End)
        # Attaches to End point. Goes to Bottom (Start height).
        att_2_end = {"x": end_pt["x"], "y": start_pt["y"]+82}
        strand_2_2 = create_strand_base(end_pt, att_2_end, color, f"{v_set_num}_2", v_set_num, "AttachedStrand", main_layer, 1)
        strands_2.append(strand_2_2)

        # Attached Strand (_3) - Bottom (Start)
        # Attaches to Start point. Goes to Top (End height).
        att_3_end = {"x": start_pt["x"], "y": end_pt["y"]-82}
        strand_2_3 = create_strand_base(start_pt, att_3_end, color, f"{v_set_num}_3", v_set_num, "AttachedStrand", main_layer, 0)
...
def main():
    output_dir = r"C:\Users\YonatanSetbon\.vscode\OpenStrandStudio\src\mxn\mxn_startings\mxn_rh"

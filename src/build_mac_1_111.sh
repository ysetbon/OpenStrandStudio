#!/bin/bash
################################################################################
# OpenStrand Studio - One-command macOS build (v1.111)
#
# Does EVERYTHING in a single run:
#   1. Builds the .app bundle with PyInstaller (OpenStrandStudio_mac.spec)
#   2. Packages the .pkg installer (build_installer_1_111.sh)
#
# Usage:
#   cd src
#   ./build_mac_1_111.sh
#
# Optional:
#   PYTHON=python3.13 ./build_mac_1_111.sh   # choose a specific interpreter
#   ./build_mac_1_111.sh --dmg               # also build the .dmg
#
...
if [ "$BUILD_DMG" = "yes" ]; then
    chmod +x build_dmg_1_111.sh
    ./build_dmg_1_111.sh
else
    chmod +x build_installer_1_111.sh
    ./build_installer_1_111.sh
fi

...
def generate_json(m, n, horizontal_gap=-26, vertical_gap=-26):
    ...
    for i in range(m):
        ...
        add_strand(main_strand["end"], 
                   {"x": main_strand["start"]["x"] - 2*vertical_gap, "y": main_strand["start"]["y"] + 2*delta_x},
                   colors[i+1], f"{i+1}_2", i+1, "AttachedStrand")
        add_strand(main_strand["start"], 
                   {"x": main_strand["end"]["x"] + 2*vertical_gap, "y": main_strand["end"]["y"] - 2*delta_x},
    ...
    for i in range(n):
        ...
        add_strand(main_strand["end"], 
                   {"x": main_strand["start"]["x"] - 2*delta_y, "y": main_strand["start"]["y"] - 2*horizontal_gap},
                   colors[m+i+1], f"{m+i+1}_2", m+i+1, "AttachedStrand")
        add_strand(main_strand["start"], 
                   {"x": main_strand["end"]["x"] + 2*delta_y, "y": main_strand["end"]["y"] + 2*horizontal_gap},
    ...
    for v_strand in new_vertical_strands:
        for h_strand in new_horizontal_strands:
            if (v_strand["layer_name"].endswith("_4") and h_strand["layer_name"].endswith("_5")) or \
               (v_strand["layer_name"].endswith("_5") and h_strand["layer_name"].endswith("_4")):
...
def main():
    output_dir = r"C:\Users\YonatanSetbon\.vscode\OpenStrandStudio\src\samples\ver 1_063\continuing_mxn_lh"

...
def generate_json(m, n, horizontal_gap=-26, vertical_gap=-26, base_spacing=114,
    ...
    for i in range(m):
        ...
        strand2 = create_strand(main_strand["start"], 
            {"x": main_strand["end"]["x"] + 1.5*vertical_gap, "y": main_strand["end"]["y"] + 1.5*vertical_gap},
        ...
        strand3 = create_strand(main_strand["end"], 
            {"x": main_strand["start"]["x"] - 1.5*vertical_gap, "y": main_strand["start"]["y"] - 1.5*vertical_gap},
    ...
    for i in range(n):
        ...
        # Create x_2
        strand2 = create_strand(main_strand["start"], 
            {"x": main_strand["end"]["x"] - 1.5*horizontal_gap, "y": main_strand["end"]["y"] + 1.5*horizontal_gap},
        ...
        # Create x_3
        strand3 = create_strand(main_strand["end"], 
            {"x": main_strand["start"]["x"] + 1.5*horizontal_gap, "y": main_strand["start"]["y"] - .15*horizontal_gap},
...
def main():
    ...
    # Length extensions remain constant
    x4_length_extension = 55
    x5_length_extension = 55

    # Loop through all possible angles (0 to 359 degrees)
    for i in range(0, 50, 5):  # vertical angles
        for j in range(0, 50, 5):  # horizontal angles
            x4_vertical_angle = i
            x5_vertical_angle = (i + 180) % 360  # Opposite direction
            x4_horizontal_angle = (j + 270) %360
            x5_horizontal_angle = (j + 90) % 360  # Opposite direction
            ...
            for m in range(1, 2):
                for n in range(1, 2):
                    ...
                    file_name = f"mxn_rh_{m}x{n}_v{i}_h{j}.json"
